...
test_that("standardize_column_names standardizes names", {
	df <- data.frame(
	...
	res <- standardize_column_names(df)
	expect_equal(names(res$data), c("first_name", "age_years"))
	expect_equal(nrow(res$name_map), 2)
})


test_that("collapse_repeated_values collapses rows by submission id", {
	df <- data.frame(
...
res <- collapse_repeated_values(df, id_col = "submissionId")
...
test_that("collapse_repeated_values handles groups that are all NA", {
	df <- data.frame(
		submissionId = c("a", "a", "b"),
		comment = c(NA, NA, "ok"),
		stringsAsFactors = FALSE
...
test_that("collapse_repeated_values handles list columns by converting to text", {
	df <- data.frame(
		submissionId = c("a", "a", "b"),
		stringsAsFactors = FALSE
	)
...
test_that("collapse_repeated_values handles data.frame columns by converting to text", {
	df <- data.frame(
		submissionId = c("a", "a", "b"),
		stringsAsFactors = FALSE
	)
	...
	expect_warning(res <- collapse_repeated_values(df, id_col = "submissionId"), NA)
	expect_equal(nrow(res$data), 2)
	expect_true(is.character(res$data$nested))
})


test_that("deduplicate_submission_rows keeps last by time column", {
	df <- data.frame(
		submissionId = c("a", "a", "b"),
	...
	res <- deduplicate_submission_rows(df, id_col = "submissionId", keep = "last")
	expect_equal(nrow(res$data), 2)
	expect_equal(res$data$value[res$data$submissionId == "a"], 2)
})


test_that("collapse_checkbox_selections merges checkbox columns", {
	df <- data.frame(
	...
	res <- collapse_checkbox_selections(df, sep = "-")
	expect_true("food-selected" %in% names(res$data))
	expect_equal(res$data[["food-selected"]], c("apple", "banana"))
})

test_that("collapse_checkbox_selections tolerates blank/NA checkbox columns", {
	df <- data.frame(
...
test_that("collapse_checkbox_selections works with logical columns", {
	df <- data.frame(
...
test_that("Cleaning helpers return updated $flat when return_flat = TRUE", {
	...
	norm <- standardize_column_names(flat_norm, return_flat = TRUE, quiet = TRUE)
	...
	resolved <- collapse_repeated_values(flat_resolve, id_col = "submission_id", return_flat = TRUE, quiet = TRUE)
	...
	dedup <- deduplicate_submission_rows(flat_dedup, id_col = "submission_id", keep = "last", return_flat = TRUE, quiet = TRUE)
	...
	compacted <- collapse_checkbox_selections(flat_compact, sep = "-", return_flat = TRUE, quiet = TRUE)
...
test_that("collapse_repeated_values collapses FoodTypes back to one row per submission and preserves totals", {
	raw <- foodtypes_raw()
	flat <- foodtypes_flat()

	resolved <- collapse_repeated_values(flat, id_col = "form-submission_id", return_flat = TRUE, quiet = TRUE)
