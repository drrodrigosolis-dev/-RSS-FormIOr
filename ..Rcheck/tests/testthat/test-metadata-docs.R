...
test_that("build_field_dictionary returns input fields with expected columns", {
	fields <- build_field_dictionary(sample_form, quiet = TRUE)
...
test_that("describe_form_schema summarizes counts", {
	res <- describe_form_schema(sample_form, quiet = TRUE)
...
test_that("build_field_dictionary can unwrap schema containers", {
	wrapped <- list(schema = sample_form)
	fields <- build_field_dictionary(wrapped, quiet = TRUE)
...
test_that("Schema root detection prefers top-level form objects", {
	...
	fields <- build_field_dictionary(wrapped, include = "all", quiet = TRUE)

	expect_true(all(c("name", "age") %in% fields$key))
})


test_that("build_field_dictionary handles missing labels without errors", {
	form <- list(
		type = "form",
		components = list(
	...
	fields <- build_field_dictionary(form, include = "all", quiet = TRUE)
...
test_that("compare_form_versions reports added, removed, and changed fields", {
	old <- sample_form
	new <- list(
		title = "Sample Form",
		components = list(
	...
	res <- compare_form_versions(old, new, quiet = TRUE)
