...
test_that("apply_submission_updates deletes rows by ID and applies updates", {
	df <- data.frame(
...
test_that("apply_submission_updates coerces values to numeric columns when possible", {
	df <- data.frame(
		submissionId = c("a", "b"),
		score = c(1, 2),
		stringsAsFactors = FALSE
...
test_that("apply_submission_updates can update a flatten_submission_records-style object", {
	flat <- list(
