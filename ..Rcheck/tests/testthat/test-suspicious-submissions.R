...
test_that("detect_suspicious_submissions detects pre/post drift and non-comparability", {
...
test_that("detect_suspicious_submissions keeps comparable groups when no drift exists", {
  set.seed(11)
  n <- 120
  df <- data.frame(
...
test_that("detect_suspicious_submissions applies BH adjustment and does not over-flag borderline fields", {
  df <- make_shifted_dataset()
