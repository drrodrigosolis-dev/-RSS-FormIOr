...
test_that("Edge dataset count_multivalue_fields highlights repeatGrid fields", {
	flat <- edge_flat()
	id_idx <- match("form-submission_id", names(flat$FlatResponses))
	out <- count_multivalue_fields(flat, id_col = id_idx)
...
test_that("Edge dataset collapse_repeated_values collapses and sums repeatGrid totals", {
	resolved <- collapse_repeated_values(edge_flat(), id_col = "form-submission_id", quiet = TRUE)
...
test_that("Edge dataset collapse_checkbox_selections builds program-selected values", {
	resolved <- edge_resolved()

	compacted <- collapse_checkbox_selections(resolved, sep = "-", drop = FALSE, quiet = TRUE)
	out <- compacted$data
...
test_that("Edge dataset summarize_response_timeline counts all submissions", {
	resolved <- edge_resolved()
