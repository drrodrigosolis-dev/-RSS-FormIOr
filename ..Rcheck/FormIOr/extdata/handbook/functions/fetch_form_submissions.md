...
- `base_url`: Character. API base URL. Default: "https://submit.digital.gov.bc.ca/app/api/v1"
  Note (CHEF): Submission endpoints use `/app/api/v1`. Metadata/schema
...
- (See `?fetch_form_submissions` for return value details.)
...
```r
# See ?fetch_form_submissions for examples
```
...
- Prev: [fetch_form_responses](fetch_form_responses.md)
- Next: [Next Function](is_audit_log_active.md)
