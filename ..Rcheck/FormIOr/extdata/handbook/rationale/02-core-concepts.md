...
2. A "flat" object returned by `flatten_submission_records()`
...
- `collapse_repeated_values()` for automatic collapsing
- `resolve_duplicate_values()` for interactive, column-by-column decisions
- `deduplicate_submission_rows()` when duplicates are true duplicates
