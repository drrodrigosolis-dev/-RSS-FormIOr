...
by `fetch_form_responses()` (or any data.frame of submissions you already have):

```r
# Option A: download from FormIO
raw <- fetch_form_responses(form_id = "your-form-id", api_key = "your-api-key")
...
# codebook <- build_data_codebook(norm, form = form_meta, include = "all")

# Summary examples (one field at a time)
summary_age <- summarize_field_distribution(flat, field = "age")
summary_program <- summarize_field_distribution(flat, field = "program")
summary_region <- summarize_field_distribution(flat, field = "region")

plot_numeric_distribution(flat, "age")
plot_categorical_summary(flat, "program")
```
