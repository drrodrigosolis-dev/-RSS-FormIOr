...
- `path`: File path for the Excel output (required; use `tempfile(fileext = ".xlsx")` in examples).
...
export_results_to_excel(df, export_path, overwrite = TRUE)
```
...
- Prev: [describe_form_schema](describe_form_schema.md)
- Next: [Next Function](build_field_dictionary.md)
