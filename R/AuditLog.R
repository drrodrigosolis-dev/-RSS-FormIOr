...
#' start_audit_log(log_file, overwrite = TRUE)
#'
#' # Run some FormIOr steps (each will append a row when possible)
#' flat <- flatten_submission_records(FoodTypes)
#' norm <- standardize_column_names(flat, quiet = TRUE)
#'
#' stop_audit_log()
#' read.csv(log_file, stringsAsFactors = FALSE)
#' }
start_audit_log <- function(file = NULL, overwrite = FALSE, append = FALSE, quiet = FALSE) {
	if (is.null(file)) {
	...
	entry <- append_audit_log_entry( # Write the initial "log started" entry.
		action = "log_started",
...
#' stop_audit_log()
#' }
stop_audit_log <- function(quiet = FALSE) {
	state <- get_audit_state() # Load current audit state.
...
maybe_prompt_audit_log <- function() {
	...
	if (tolower(answer) %in% c("y", "yes")) {
		...
		start_audit_log(file = log_file, overwrite = FALSE, append = append_mode, quiet = TRUE) # Start logging silently.
...
#' @return The result of `append_audit_log_entry()` (invisibly) or `NULL`.
#' @keywords internal
maybe_write_audit <- function(action, details = NULL, data = NULL) {
	...
	append_audit_log_entry( # Append a log row.
		action = action,
		details = details,
		file = state$file,
		data = data,
