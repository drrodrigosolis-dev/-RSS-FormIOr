...
launch_formior_addin <- function() {
    ...
    miniUI::miniTabstripPanel(
      miniUI::miniTabPanel(
        ...
        miniUI::miniContentPanel(
          ...
          shiny::actionButton("run_get", "Run fetch_form_responses"),
          shiny::actionButton("undo_get", "Undo fetch_form_responses")
        )
      ),
      miniUI::miniTabPanel(
        "Flatten",
        miniUI::miniContentPanel(
          ...
          shiny::actionButton("run_flatten", "Run flatten_submission_records"),
          shiny::actionButton("undo_flatten", "Undo flatten_submission_records"),
      ...
      miniUI::miniTabPanel(
        ...
        miniUI::miniContentPanel(
          ...
          shiny::actionButton("run_norm", "Run standardize_column_names"),
          shiny::actionButton("undo_norm", "Undo standardize_column_names")
        )
      ),
      miniUI::miniTabPanel(
        "Resolve Repeats",
        miniUI::miniContentPanel(
          ...
          shiny::actionButton("run_resolve", "Run collapse_repeated_values"),
          shiny::actionButton("undo_resolve", "Undo collapse_repeated_values")
        )
      ),
      miniUI::miniTabPanel(
        "Codebook",
        miniUI::miniContentPanel(
          ...
          shiny::actionButton("run_codebook", "Run build_data_codebook"),
          shiny::actionButton("undo_codebook", "Undo build_data_codebook")
        )
      ),
      miniUI::miniTabPanel(
        "Export",
        miniUI::miniContentPanel(
          ...
          shiny::actionButton("run_export", "Run export_results_to_excel"),
          shiny::actionButton("undo_export", "Undo export_results_to_excel")
        )
      ),
      miniUI::miniTabPanel(
        "Summary",
        miniUI::miniContentPanel(
          ...
          shiny::actionButton("run_summary", "Run summarize_field_distribution"),
          shiny::actionButton("undo_summary", "Undo summarize_field_distribution")
        )
      ),
      miniUI::miniTabPanel(
        "Field Dictionary",
        miniUI::miniContentPanel(
          ...
          shiny::actionButton("run_fd", "Run build_field_dictionary"),
          shiny::actionButton("undo_fd", "Undo build_field_dictionary"),
          shiny::hr(),
  ...
  server <- function(input, output, session) {
    ...
    resolve_fd_source <- function(text) {
      text <- trimws(text)
      if (!nzchar(text)) {
        status("build_field_dictionary error: source is empty.")
        return(NULL)
      }
      ...
      status("build_field_dictionary error: source must be an object name, JSON file path, or JSON string.")
      NULL
    }
    ...
    shiny::observeEvent(input$run_get, {
      ...
      out <- run_with_audit(
        ...
        fn = function() {
          tryCatch(
            ...
            error = function(e) {
              status(paste("fetch_form_responses error:", e$message))
              NULL
            }
          )
        }
      ...
      if (!is.null(out)) {
        ...
        status(paste0("fetch_form_responses complete. Saved to '", target_name, "' in Global Environment."))
    ...
    shiny::observeEvent(input$run_flatten, {
      ...
      if (!nzchar(source_name) || !exists(source_name, envir = user_env)) {
        status("flatten_submission_records error: source object not found in Global Environment.")
        return()
      }
      ...
      out <- run_with_audit(
        ...
        fn = function() {
          tryCatch(
            flatten_submission_records(source_obj),
            error = function(e) {
              status(paste("flatten_submission_records error:", e$message))
              NULL
            }
          )
        }
      ...
      if (!is.null(out)) {
        assign(target_name, out, envir = user_env)
        status(paste0("flatten_submission_records complete. Saved to '", target_name, "' in Global Environment."))
        set_preview(out, target_name)
      }
    })
    ...
    shiny::observeEvent(input$run_norm, {
      ...
      if (!nzchar(source_name) || !exists(source_name, envir = user_env)) {
        status("standardize_column_names error: source object not found in Global Environment.")
        return()
      }
      ...
      out <- run_with_audit(
        ...
        fn = function() {
          tryCatch(
            ...
            error = function(e) {
              status(paste("standardize_column_names error:", e$message))
              NULL
            }
          )
        }
      ...
      if (!is.null(out)) {
        assign(target_name, out, envir = user_env)
        status(paste0("standardize_column_names complete. Saved to '", target_name, "' in Global Environment."))
        set_preview(out, target_name)
      }
    })
    ...
    shiny::observeEvent(input$run_resolve, {
      ...
      if (!nzchar(source_name) || !exists(source_name, envir = user_env)) {
        status("collapse_repeated_values error: source object not found in Global Environment.")
        return()
      }
      ...
      out <- run_with_audit(
        ...
        fn = function() {
          tryCatch(
            ...
            error = function(e) {
              status(paste("collapse_repeated_values error:", e$message))
              NULL
            }
          )
        }
      ...
      if (!is.null(out)) {
        assign(target_name, out, envir = user_env)
        status(paste0("collapse_repeated_values complete. Saved to '", target_name, "' in Global Environment."))
        set_preview(out, target_name)
      }
    })
    ...
    shiny::observeEvent(input$run_codebook, {
      ...
      if (!nzchar(source_name) || !exists(source_name, envir = user_env)) {
        status("build_data_codebook error: source object not found in Global Environment.")
        return()
      }
      ...
      out <- run_with_audit(
        ...
        fn = function() {
          tryCatch(
            ...
            error = function(e) {
              status(paste("build_data_codebook error:", e$message))
              NULL
            }
          )
        }
      ...
      if (!is.null(out)) {
        assign(target_name, out, envir = user_env)
        status(paste0("build_data_codebook complete. Saved to '", target_name, "' in Global Environment."))
        set_preview(out, target_name)
      }
    })
    ...
    shiny::observeEvent(input$run_export, {
      source_obj <- resolve_source_obj(input$export_source, "export_results_to_excel")
      ...
      if (!nzchar(path)) {
        status("export_results_to_excel error: output path is empty.")
        return()
      }
      ...
      export_info <- tryCatch(
        ...
        error = function(e) {
          status(paste("export_results_to_excel error:", e$message))
          NULL
        }
      )
      ...
      assign(target_name, out, envir = user_env)
      status(paste0("export_results_to_excel complete. Saved to '", target_name, "' in Global Environment."))
      set_preview(out, target_name)
    })
    ...
    shiny::observeEvent(input$run_summary, {
      ...
      df <- resolve_source_df(source_name, "summarize_field_distribution")
      if (is.null(df)) return()
      source_obj <- get(source_name, envir = user_env)
      ...
      if (is.null(field) || !nzchar(as.character(field))) {
        status("summarize_field_distribution error: field is required.")
        return()
      }
      ...
      out <- run_with_audit(
        ...
        fn = function() {
          tryCatch(
            ...
            error = function(e) {
              status(paste("summarize_field_distribution error:", e$message))
              NULL
            }
          )
        }
      ...
      if (!is.null(out)) {
        assign(target_name, out, envir = user_env)
        status(paste0("summarize_field_distribution complete. Saved to '", target_name, "' in Global Environment."))
    ...
    shiny::observeEvent(input$run_fd, {
      ...
      out <- run_with_audit(
        ...
        fn = function() {
          tryCatch(
            ...
            error = function(e) {
              status(paste("build_field_dictionary error:", e$message))
              NULL
            }
          )
        }
      ...
      if (!is.null(out)) {
        assign(target_name, out, envir = user_env)
        field_dict_full(out)
        status(paste0("build_field_dictionary complete. Saved to '", target_name, "' in Global Environment."))
        set_preview(out, target_name)
      }
    })
