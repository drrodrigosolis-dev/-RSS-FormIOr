...
#' @param x list with responses, typically obtained through the fetch_form_responses() function.
#'
#' @returns list with
#'	* `FlatResponses` which is the entire dataset obtained from fetch_form_responses() but, where all the nested lists are flattened into a bidimensional data frame. nested lists will take their name by using their parent name as a prefix, then a "-" separator, and then their own name.
...
flatten_submission_records <- function(x) {
	LineData<-list()
	for(j in 1:nrow(x)){
		RespondentLine <- x[j, ]
