...
#' or a list returned by [fetch_form_responses()] / [flatten_submission_records()].
...
#' \dontrun{
#'   export_results_to_excel(df, tempfile(fileext = ".xlsx"), overwrite = TRUE)
#' }
...
#' It can optionally use a FormIO schema (via [build_field_dictionary()]) to add labels,
...
#'   [build_field_dictionary()]). If provided, labels and sections are included.
...
build_data_codebook <- function(
	...
	if (!is.null(form)) {
		...
		field_info <- build_field_dictionary(form, include = include, quiet = TRUE) # Build schema dictionary.
	}
	...
	if (audit_depth == 1) {
		maybe_write_audit("build_data_codebook", data = data) # Record the action.
	}
...
#'   quiet = TRUE
#' )
append_audit_log_entry <- function(
		action,
		details = NULL,
		file = NULL,
		data = NULL,
	...
	if (is.null(file)) {
		stop("file is required. Provide a file path or start an audit log with start_audit_log().")
	}
...
#'
#' @param data Data frame, list of data frames, or flatten_submission_records list.
