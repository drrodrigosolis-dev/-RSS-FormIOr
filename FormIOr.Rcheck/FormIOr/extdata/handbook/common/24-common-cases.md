...
```r
ExportToExcel(flat$FlatResponses, path = file.path(tempdir(), "FormIOr_output.csv"), overwrite = TRUE)
```
...
```r
StartAuditLog(file.path(tempdir(), "audit_log.csv"), append = TRUE)
# ...run cleaning steps...
StopAuditLog()
```
...
**Answer:** Use `FlagSuspiciousSubmissions()` with a cutoff timestamp. If the
