...
| [AskCredentials](AskCredentials.md) | Collects the Form ID and API key in an interactive session and caches them for this R session. |
...
| [FlagSuspiciousSubmissions](FlagSuspiciousSubmissions.md) | Evaluates whether pre/post time groups are statistically comparable and produces row-level suspiciousness flags. This is designed for workflows where a survey changed mid-collection (for example, f... |
| [FlattenSubmissions](FlattenSubmissions.md) | This function will flatten submissions coming from a FormIO list that has nested elements |
| [FormIOrAddin](FormIOrAddin.md) | Provides a minimal point-and-click interface for key FormIOr functions. |
...
| [GetFormMetadata](GetFormMetadata.md) | Fetches form metadata from the API and retries the alternate base URL when parsing fails (CHEF compatibility behavior). |
| [GetResponses](GetResponses.md) | Downloads response export JSON from a form endpoint and returns parsed submissions by default. |
| [GetSubmissions](GetSubmissions.md) | Fetches submission-level metadata (not full response payloads) from a 'FormIO' endpoint. |
...
| [RenameCols](RenameCols.md) | Renames columns in a list returned by FlattenSubmissions(). You can run it interactively (prompt for each column) or non-interactively with rename_map. |
