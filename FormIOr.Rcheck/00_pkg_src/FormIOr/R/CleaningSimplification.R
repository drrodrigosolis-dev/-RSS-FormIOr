...
#'   \item{flat}{If `return_flat = TRUE`, the updated FlattenSubmissions-style list (for workflows that expect `FlatResponses`)}
#' }
#'
#' @export
#'
...
NormalizeColumnNames <- function(
	...
	if (audit_depth == 1) {
		maybe_write_audit("NormalizeColumnNames", data = out$data) # Record normalization action.
	}
...
ResolveRepeats <- function(
	...
	if (audit_depth == 1) {
		maybe_write_audit("ResolveRepeats", data = out$data) # Record repeat resolution.
	}
...
DeduplicateSubmissions <- function(
	...
	if (audit_depth == 1) {
		maybe_write_audit("DeduplicateSubmissions", data = out$data) # Record deduplication.
	}
...
ReviewDuplicateSubmissions <- function(
	...
	if (length(dup_groups) == 0) {
		...
		if (audit_depth == 1) {
			maybe_write_audit("ReviewDuplicateSubmissions", details = "no duplicates", data = df) # Record no-op review.
		}
		return(out)
	}
	...
	for (grp in dup_groups) {
		...
		if (audit_depth == 1) {
			...
			maybe_write_audit("ReviewDuplicateSubmissions", details = detail, data = df[rows_idx, , drop = FALSE]) # Record per-group decision.
		}
	}
...
CompactSelections <- function(
	...
	if (audit_depth == 1) {
		maybe_write_audit("CompactSelections", data = out$data) # Record compaction action.
	}
...
#'
#' @param x Data frame or FlattenSubmissions list.
