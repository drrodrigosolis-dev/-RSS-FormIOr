...
#'
#' Renames columns in a list returned by [FlattenSubmissions()].
#' You can run it interactively (prompt for each column) or non-interactively
#' with `rename_map`.
#'
#' @param x List from [FlattenSubmissions()] containing `FlatResponses` and
#'   `ColumnNames`.
...
RenameCols <- function(x, NamesDF = TRUE, renameDF = TRUE, rename_map = NULL, quiet = FALSE) {
  ...
  if (is.null(rename_map) && !interactive()) {
    stop("RenameCols() is interactive when rename_map is NULL. Provide rename_map for non-interactive use.")
  }
