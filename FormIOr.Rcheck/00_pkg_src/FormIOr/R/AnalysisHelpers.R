...
SummaryByField <- function(
	...
	if (audit_depth == 1) {
		maybe_write_audit("SummaryByField", details = paste0("field=", field_name), data = df) # Record the summary action.
	}
...
PlotHistogram <- function(
	...
	if (audit_depth == 1) {
		maybe_write_audit("PlotHistogram", details = paste0("field=", field_name), data = df) # Record the plot action.
	}
...
PlotBarSummary <- function(
	...
	summary <- SummaryByField( # Reuse summary logic for counts and percents.
		x = x,
		field = field,
		top_n = top_n,
	...
	if (audit_depth == 1) {
		maybe_write_audit("PlotBarSummary", details = paste0("field=", summary$field), data = x) # Record the plot action.
	}
...
PlotWordcloud <- function(
	...
	if (audit_depth == 1) {
		maybe_write_audit("PlotWordcloud", details = paste0("field=", paste(field_names, collapse = ", ")), data = df) # Record the plot action.
	}
...
PlotResponseTimeline <- function(
	...
	out <- ResponseTimeline( # Reuse timeline logic for data prep.
		x = x,
		date_col = date_col,
		interval = interval,
		tz = tz,
	...
	if (audit_depth == 1) {
		maybe_write_audit("PlotResponseTimeline", details = paste0("interval=", interval), data = x) # Record the plot action.
	}
