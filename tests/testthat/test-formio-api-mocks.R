...
test_that("fetch_form_responses errors when credentials are unavailable in non-interactive mode", {
	...
	expect_error(fetch_form_responses(form_id = NULL, api_key = NULL), "not available")
})


test_that("fetch_form_responses returns parsed submissions and supports content.only modes", {
	ns <- asNamespace("FormIOr")
	state <- get(".formior_state", envir = ns)
	prev <- state$Form_Info
	on.exit({
...
test_that("fetch_form_submissions returns cleaned metadata with version mapping (mocked HTTP)", {
	ns <- asNamespace("FormIOr")
	state <- get(".formior_state", envir = ns)
	prev <- state$Form_Info
	on.exit({
	...
	testthat::local_mocked_bindings(
		...
		add_headers = function(...) NULL,
		timeout = function(...) NULL,
		fetch_form_metadata = mock_meta,
		.package = "FormIOr"
	)

	out <- fetch_form_submissions(
		base_url = "https://example.com/app/api/v1",
		form_id = "FORM",
		api_key = "KEY",
		content.only = TRUE,
...
test_that("fetch_form_metadata retries alternate base_url when JSON parsing fails (mocked HTTP)", {
	ns <- asNamespace("FormIOr")
	state <- get(".formior_state", envir = ns)
	prev <- state$Form_Info
	on.exit({
...
test_that("fetch_form_metadata errors when both base URLs return invalid JSON (mocked HTTP)", {
	calls <- list()
