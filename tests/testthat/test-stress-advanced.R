test_that("build_data_codebook handles list columns by converting to text", {
	flat <- edge_flat()
	cb <- build_data_codebook(flat$FlatResponses, quiet = TRUE)
...
test_that("rename_columns_from_dictionary applies a partial rename_map and keeps other names", {
	flat <- edge_flat()
	map <- data.frame(
	...
	out <- rename_columns_from_dictionary(flat, rename_map = map, quiet = TRUE)
...
test_that("export_results_to_excel sanitizes long/illegal sheet names for delimited output", {
	df1 <- data.frame(a = 1)
	df2 <- data.frame(a = 2)
	sheets <- structure(
		list(df1, df2),
	...
	out <- export_results_to_excel(sheets, path = out_path, overwrite = TRUE, quiet = TRUE)
...
test_that("export_results_to_excel errors when output exists and overwrite is FALSE", {
	df <- data.frame(a = 1)
	out_path <- tempfile(fileext = ".csv")
	export_results_to_excel(df, path = out_path, overwrite = TRUE, quiet = TRUE)

	expect_error(export_results_to_excel(df, path = out_path, overwrite = FALSE, quiet = TRUE), "already exist")
})


test_that("apply_submission_updates updates list columns and preserves list type", {
...
test_that("review_duplicate_rows supports dropping an entire group", {
	df <- data.frame(
