test_that("count_multivalue_fields counts distinct values per submission", {
	df <- data.frame(
	...
	out <- count_multivalue_fields(flat, id_col = 1)
...
test_that("count_multivalue_fields flags repeated editGrid fields in FoodTypes", {
	flat <- foodtypes_flat()
