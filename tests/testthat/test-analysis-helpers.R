...
test_that("summarize_field_distribution summarizes numeric fields", {
	df <- data.frame(score = c(1, 2, 3, NA))

	res <- summarize_field_distribution(df, "score", quiet = TRUE)
...
test_that("summarize_field_distribution summarizes categorical fields", {
	df <- data.frame(fruit = c("Apple", "Apple", "Banana", NA))

	res <- summarize_field_distribution(df, "fruit", include_na = TRUE, quiet = TRUE)
...
test_that("tabulate_field_by_group builds counts and percents", {
	df <- data.frame(
	...
	res <- tabulate_field_by_group(df, "region", "program", percent = "overall", quiet = TRUE)
...
test_that("summarize_response_timeline counts by day and fills empty dates", {
	df <- data.frame(
		created = c("2024-01-01", "2024-01-01", "2024-01-03"),
		stringsAsFactors = FALSE
	)

	res <- summarize_response_timeline(df, date_col = "created", interval = "day", include_empty = TRUE, quiet = TRUE)
...
test_that("summarize_response_timeline respects explicit start/end bounds and keeps edge zeroes", {
	df <- data.frame(
		created = c("2024-01-01", "2024-01-03"),
		stringsAsFactors = FALSE
	)

	res <- summarize_response_timeline(
		df,
		date_col = "created",
		interval = "day",
		start = "2023-12-30",
...
test_that("summarize_response_timeline buckets by hour and fills empty hours", {
	df <- data.frame(
		created = c("2024-01-01 10:15:00", "2024-01-01 10:45:00", "2024-01-01 12:00:00"),
		stringsAsFactors = FALSE
	)

	res <- summarize_response_timeline(df, date_col = "created", interval = "hour", tz = "UTC", include_empty = TRUE, quiet = TRUE)
...
test_that("summarize_response_timeline works on FoodTypes (after collapse_repeated_values) and counts submissions", {
	raw <- foodtypes_raw()
	resolved <- foodtypes_resolved()

	tl <- summarize_response_timeline(
		resolved,
